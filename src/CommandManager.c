...
{ // constructor
  ...
  if (this->_originalCmd == NULL) {// for exception safety
    GarbagCollect(this->_commands);
    return NULL;
  }
...
{ // destructor
  GarbagCollect(this->args);
  GarbagCollect(this->_originalCmd);
  GarbagCollect(this->_commands);
...
void CommandProcess(ArgsMgr* const this)
{
  this->lastCmdType = nil;
  ...
  if (pipeFlag != -1) {
    ...
    this->lastCmdType = SyntacticParser(this->args, this->argc);
    if (this->lastCmdType > buildin)
      executeFlag = BuildinCmds(this->args, this->argc, this->lastCmdType);
    else if (this->lastCmdType < buildin)
      executeFlag = ExternalCmds(this->args, this->argc, this->lastCmdType);
  }
